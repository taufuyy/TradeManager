...
class Dashboard(ctk.CTk):
    ...
    def _build_action_grid(self, parent):
        ...
        self._entry_sl = ctk.CTkEntry(b1_inner, height=35, width=80, font=(FONT_MONO, 12), placeholder_text="0.00")
        ...
        self._entry_tp = ctk.CTkEntry(b1_inner, height=35, width=80, font=(FONT_MONO, 12), placeholder_text="0.00")
        ...
        self._btn_apply_sltp.grid(row=3, column=0, columnspan=4, sticky="ew", pady=5)
        
        # Row 4: BE Offset + Smart BE
        ctk.CTkLabel(b1_inner, text="BE Offset:", font=(FONT_FAMILY, 12), text_color=TEXT_SECONDARY).grid(row=4, column=0, sticky="w", padx=(0, 4), pady=5)
        ...
        self._entry_be_offset.grid(row=4, column=1, sticky="w", padx=(0, 4), pady=5)
        ctk.CTkLabel(b1_inner, text="pips", font=(FONT_FAMILY, 11), text_color=TEXT_SECONDARY).grid(row=4, column=2, sticky="w", padx=(0, 4), pady=5)
        ...
        self._btn_smart_be.grid(row=4, column=3, sticky="ew", pady=5)
        
        # Row 5: Trailing Stop + Start Trailing
        ctk.CTkLabel(b1_inner, text="Trailing Stop:", font=(FONT_FAMILY, 12), text_color=TEXT_SECONDARY).grid(row=5, column=0, sticky="w", padx=(0, 4), pady=5)
        ...
        self._entry_trail.grid(row=5, column=1, sticky="w", padx=(0, 4), pady=5)
        ...
        self._btn_trail.grid(row=5, column=3, sticky="ew", pady=5)
        ...
        b3_inner.grid_columnconfigure(1, weight=0)
        b3_inner.grid_columnconfigure(2, weight=0)
        b3_inner.grid_columnconfigure(3, weight=0)
        ...
        b3_inner2.grid_columnconfigure(1, weight=0)
        b3_inner2.grid_columnconfigure(2, weight=0)
        b3_inner2.grid_columnconfigure(3, weight=0)
    ...
    def _do_refresh(self):
        ...
        if connected:
            ...
            self._lbl_conn.configure(text="Connected", text_color=GREEN_BRIGHT)
        else:
        ...
        if total_layers == 0:
            ...
            rv["Margin:"].configure(text=format_with_idr(margin_val, self.cfg), text_color=zone_color)
            rv["Margin Lvl:"].configure(text=f"{ml:.1f}%", text_color=zone_color)
    ...
    def _on_apply_sltp(self):
        ...
        if mode == "TP ALL":
            ...
            msg_tuples = [(f"Apply SL={sl} & TP={tp} to ALL layers?", TEXT_PRIMARY)]
            ...
            for tier in tiers:
                if tier["tp"] > 0:
                    msg_tuples.append((f"- {tier['pct']}% layer ditutup pada {tier['tp']}", GREEN_BRIGHT))
                else:
    ...
    def _on_selective_close(self):
        try:
            qty = int(self._entry_close_qty.get() or 1)
        except ValueError:
            qty = 1
        ...
        self.cfg.set_many({
            "selective_close_qty": qty,
            "selective_close_direction": direction,
            "selective_close_sort": sort_order,
        })
        self._confirm_and_execute(
            "Selective Close",
            f"Close {qty} {direction} position(s) sorted by '{sort_order}'?",
    ...
    def _on_arm_target_profit_switch(self):
        ...
        if armed:
            try:
                usc = float(self._var_tp_usc.get() or 0)
                idr = float(self._var_tp_idr.get().replace(",", "") or 0)
            except ValueError:
            ...
            tkinter.messagebox.showinfo("Auto-Close Aktif", f"Auto-Close AKTIF dengan Target Profit: {usc} USD / Rp {idr:,.0f}")
        else:
    ...
    def _on_arm_target_loss_switch(self):
        ...
        if armed:
            try:
                usc = float(self._var_tl_usc.get() or 0)
                idr = float(self._var_tl_idr.get().replace(",", "") or 0)
            except ValueError:
            ...
            import tkinter.messagebox
            tkinter.messagebox.showinfo("Auto-Close Aktif", f"Auto-Close AKTIF dengan Target Loss: {usc} USD / Rp {idr:,.0f}")
        else:
    ...
    def _draw_tick_chart(self, state: dict):
        self._canvas_meter.delete("all")
        w = self._canvas_meter.winfo_width()
        h = self._canvas_meter.winfo_height()
        if w <= 1 or h <= 1:
            w, h = 300, 150 # fallback
            
        if not self._price_history:
            return
            
        bid_price = self._price_history[-1]
        ask_price = state.get("ask", 0.0)
        avg_price = state.get("avg_price_combined", 0.0)
        actual_mc_price = state.get("mc_price", 0.0)
        
        # Prefer simulated MC price if it's set, else use the actual one
        mc_price_to_draw = self._simulated_mc_price if self._simulated_mc_price > 0 else actual_mc_price
        ...
        if mc_price_to_draw > 0:
            prices.append(mc_price_to_draw)
        ...
        def get_y(price):
            ...
            return h - padding_y - (ratio * usable_h)
            
        # Draw subtle grid lines
        for i in range(1, 5):
            y_grid = h * (i / 5)
            self._canvas_meter.create_line(0, y_grid, w, y_grid, fill="#2a3038", width=1, dash=(2, 4))
        
        for i in range(1, 8):
            x_grid = w * (i / 8)
            self._canvas_meter.create_line(x_grid, 0, x_grid, h, fill="#2a3038", width=1, dash=(2, 4))
            
        # Right axis margin
        axis_margin = 60
        chart_w = w - axis_margin
        
        # Draw line chart for price history
        points = []
        x_step = chart_w / (self._price_history.maxlen - 1)
        start_idx = self._price_history.maxlen - len(self._price_history)
        
        for i, price in enumerate(self._price_history):
            x = (start_idx + i) * x_step
            y = get_y(price)
            points.append((x, y))
            
        if len(points) > 1:
            flat_points = [coord for pt in points for coord in pt]
            self._canvas_meter.create_line(flat_points, fill="#00ff00", width=2) # Lime green line
            
        y_bid = get_y(bid_price)
        
        # Ask Price Line (Red dashed)
        if ask_price > 0:
            y_ask = get_y(ask_price)
            self._canvas_meter.create_line(0, y_ask, chart_w, y_ask, fill=RED, width=1, dash=(4, 2))
            self._canvas_meter.create_rectangle(chart_w, y_ask - 8, w, y_ask + 8, fill=RED, outline="")
            self._canvas_meter.create_text(chart_w + 5, y_ask, text=f"{ask_price:.2f}", fill=BG_DARK, anchor="w", font=(FONT_FAMILY, 9, "bold"))
        
        # Bid Price Line (Cyan/Blue dashed)
        self._canvas_meter.create_line(0, y_bid, chart_w, y_bid, fill="#58a6ff", width=1, dash=(4, 2))
        self._canvas_meter.create_rectangle(chart_w, y_bid - 8, w, y_bid + 8, fill="#58a6ff", outline="")
        self._canvas_meter.create_text(chart_w + 5, y_bid, text=f"{bid_price:.2f}", fill=BG_DARK, anchor="w", font=(FONT_FAMILY, 9, "bold"))
        self._simulated_mc_price = result.get("new_mc_price", 0.0)
        
        # Update simulator chart lines
        s = self.mt5.get_state() if self.mt5 else {}
        self.sim_chart.update_lines(
            ask=s.get("ask", 0),
            bid=s.get("bid", 0),
            avg=result.get("new_avg_price", s.get("avg_price_combined", 0)),
    ...
    def _confirm_and_execute(self, title: str, message, action, screenshot_label: str = ""):
        ...
        for text, color in message:
            ctk.CTkLabel(dialog, text=text, font=(FONT_FAMILY, 12, "bold" if color != TEXT_PRIMARY else "normal"),
                         text_color=color, wraplength=380, justify="center").pack(pady=(0, 4))
        ...
        dialog.update_idletasks()
        req_width = max(420, dialog.winfo_reqwidth())
        req_height = max(180, dialog.winfo_reqheight() + 10)
